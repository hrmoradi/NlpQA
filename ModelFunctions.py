...
class MyTFQuestionAnswering(TFPreTrainedModel, TFQuestionAnsweringLoss):
    def __init__(self, modelName, pathName, *inputs, **kwargs):
        ...
        if modelName.lower() == "llama":
            config = LlamaConfig.from_pretrained(pathName)
        else:
        ...
        if modelName == "llama":
            self.model = LlamaModel.from_pretrained(pathName, config=config)
        else:
            self.model = TFAutoModel.from_pretrained(pathName, config=config, from_pt=modelName in models_with_no_tf)
